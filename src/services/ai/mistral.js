 ...
 */
async function requestAnalysis(signedUrl, transcription, doChecklist, dontChecklist, language) {
 ...
 * @returns {Promise<Object>} - Analysis results
 */
async function analyzeDirectFromAudio(filePath, transcription, doChecklist, dontChecklist, language) {
  try {
    const fileId = await uploadAudio(filePath);
    const signedUrl = await getSignedUrl(fileId);
    const analysis = await requestAnalysis(signedUrl, transcription, doChecklist, dontChecklist, language);
    return analysis;
  } catch (error) {
